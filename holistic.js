...
function calcTotalHours() {
  ...
  let startD, endD;
  if (isMathayom) {
    // มัธยม → นับรายภาคเรียนตาม App.gradeTerm
    const term = App.gradeTerm || 1;
    startD = App.termDates[`t${term}_start`];
    endD   = App.termDates[`t${term}_end`];
  } else {
    // ประถม → นับทั้งปี (t1_start ถึง t2_end)
    startD = App.termDates['t1_start'];
    endD   = App.termDates['t2_end'];
  }
