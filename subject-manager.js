...
const SubjectMgr = {
  ...
  loadClass(cls) {
    ...
    const seqSet = (App.seqSubjects && App.seqSubjects[cls])
                   ? new Set(App.seqSubjects[cls])
                   : new Set();
    this.subjects = _smDetectSeq_(raw, cls, seqSet);
    this.render();
  },
  ...
  async save() {
    ...
    try {
      ...
      // อัปเดต App.seqSubjects ใน memory
      if (!App.seqSubjects) App.seqSubjects = {};
      App.seqSubjects[this.cls] = [...seqSubjects];

      // อัปเดต App.config.isSeq ทันที ถ้าวิชาที่กำลังเปิดอยู่เป็นวิชาต่อเนื่อง
      const currentSubj = (typeof App !== 'undefined' && App.loadedSubject) || '';
      if (currentSubj && App.config !== undefined) {
        const isCurrentSeq = seqSubjects.includes(currentSubj);
        if (isCurrentSeq) {
          App.config.isSeq = true;
        } else {
          delete App.config.isSeq;
        }
      }
...
function _smDetectSeq_(rawList, cls, savedSeqSet) {
  const seqNums  = CLASS_SEQ_NUMS[cls];
  const seqSaved = savedSeqSet || new Set(); // ชื่อวิชาที่บันทึกว่าเป็น seq ไว้แล้ว
  const result   = [];
  const seen     = new Set();
  ...
  for (const raw of rawList) {
    ...
    if (m) {
      ...
      const isSeqByNum  = seqNums && (num === seqNums.t1 || num === seqNums.t2);
      const isSeqSaved  = seqSaved.has(base);
      seen.add(base);
      result.push({ name: base, isSeq: !!(isSeqByNum || isSeqSaved) });
    } else {
      if (seen.has(name)) continue;
      seen.add(name);
      // ชื่อไม่มีเลข → ดูจาก savedSeqSet
      result.push({ name, isSeq: seqSaved.has(name) });
    }
  }
  return result;
}
...
function toggleSubjectManager() { SubjectMgr.init(); } // legacy — ใช้ผ่าน tab แล้ว
